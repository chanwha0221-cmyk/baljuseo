...
/* 🚩 업체 특성 깃발 — 칸 매핑(headers)과 별개로, 그 업체 규칙들 중 **하나라도** 켜 둔 값을 본다 (2026-09-15).
   예: 티알에스큐 {emptyOtel:'rcv'} = 주문처 연락처가 비면 받는분 연락처. 양식 규칙이 여러 개여도 깃발은 업체 단위다. */
function vflag(key){
  const who = (typeof amMaster === 'function' && amMaster())
    ? (FOR || {}) : ((typeof ME !== 'undefined' && ME) ? ME : {});
  const id = pkey(S(who.id || ''));
  if(!id) return undefined;
  const hit = VRULES.find(r => pkey(S(r.account_id)) === id && r.rule && r.rule[key] !== undefined);
  return hit ? hit.rule[key] : undefined;
}
function vrule(head){
  const who = (typeof amMaster === 'function' && amMaster())
    ? (FOR || {}) : ((typeof ME !== 'undefined' && ME) ? ME : {});
...
function buildOut(){
  ...
  groups.forEach(g => {
    ...
    /* ☎ 주문처 연락처 규칙 (홍팀장 2026-09-15 확정 — "번호가 채워져 있으면 그걸 써, 비어 있는 것만 생각하면 된다"):
         ① 파일에 번호가 있으면 그 번호  ② 비어 있으면 원래 업체(주문처) 연락처
         ③ 예외는 **업체 규칙(Supabase vendor_rules)의 emptyOtel:'rcv'** 인 업체만 — 비어 있으면 받는분 연락처 (티알에스큐).
       🔴 업체 특성은 코드에 박지 말고 그 업체 규칙에 저장한다. 처음엔 「주문처=받는분이면 받는분 번호」를 모든 업체 공통으로 박았다가 질책받았다. */
    const rcvTel = (!fTel && biz && vflag('emptyOtel') === 'rcv') ? tel : '';
    ...
    const solo = [], rest = [];
    items.forEach(it => {
      /* 🔴🔴 2026-09-14 홍팀장 확정 — 1개씩 나누는 건 **단독(무) 무료배송 상품만.** 「합포장 안 되는 상품」 목록(NOHAP)으로 나누지 않는다.
         "단독(무)에 있는 상품만 그렇게 하라고 했지 온 동네방네 합포장을 풀고 있어" / "비만 바다장어는 합포장이 되는 건데".
         그 목록은 8/28 [합포장 안 됨으로 지정] 버튼으로 쌓였다(원비씨가 바다장어 2종을 눌러 넣음) — 버튼도 숨겼다.
...
/* 🔴 2026-09-15 티알에스큐 — 파일에 「주문처 연락처」가 이미 들어 있는데 5곳 전부 "이 출고지 연락처 넣으라"고 물었다
   (홍팀장: "파일에 있잖아, 들어가 있는 거 사용해"). 번호가 이미 정해지는 이름은 묻지 않는다:
     ...
     ② 업체 규칙이 emptyOtel:'rcv'(티알에스큐) → 비어 있으면 받는분 연락처로 나간다(buildOut 참고) */
function shipsInRows(){
  const me = orderer(), out = [];
  const byName = new Map();
  const rcvRule = vflag('emptyOtel') === 'rcv';
  ROWS.forEach(r => {
    const b = S(r.biz);
    if(!b) return;
    ...
    const telDecided = S(r.otel) || (rcvRule && S(r.tel));
...
function paintOut(ok, bad){
  ...
  /* 🚫 2026-09-14 — 버튼 숨김. 1개씩 나누기는 단독(무)만(buildOut 참고). 이 버튼이 바다장어를 「합포장 안 됨」으로 만들었다. */
...
document.addEventListener('visibilitychange', () => { if(!document.hidden) setTimeout(pollClaims, 600); });
...
const HDR_MAP = {
  ...
  qty : ['수량', '주문수량', '개수', '구매수'],
  /* 🏷 '성함' — 티알에스큐 (홍팀장 2026-09-04).
     이 업체는 자기 시스템에서 메일로만 발주를 주고, 우리가 대신 넣는다. 대신 파일을
     **우리 발주 시트 칸 이름 그대로**(주문처·주문처 주소·주문처 연락처·창고명·상품명·수량·
     성함·주소·연락처·배송메세지/특이사항) 맞춰서 준다. 그런데 정작 우리가 그 '성함'을 몰라
     머리글 줄 자체를 못 찾고 통째로 변환기로 넘어갔다.
     ⚠️ 맨 뒤에 둔다 — '받는분'·'수취인'이 같이 있으면 그쪽이 먼저다.
     ⚠️ 이 목록은 머리글 **줄을 찾는 조건**이기도 하다. 그래서 업체 규칙(vendor_rules)에만
        적어서는 못 읽는다 — 줄을 못 찾으면 규칙을 볼 기회조차 없다. */
  /* 🏷 '수령자명'·'수령자' — 두름 (홍팀장 2026-09-16). 이 이름이 없어서 **머리글 줄 자체를 못 찾고**
     파일이 통째로 옛 변환기로 넘어갔다(13칸 파일을 9칸으로 알고 읽는다 = 그럴듯한 쓰레기).
     업체 규칙에만 적어서는 못 고친다 — 줄을 못 찾으면 규칙을 볼 기회조차 없다(위 '성함'과 같은 이유). */
  /* ⚠️ '수령자'(맨몸)는 넣지 않는다 — 앞글자 매칭이라 '수령자휴대폰번호'를 받는분으로 집는다.
     실제로 온 칸 이름만 넣는다(§3-3 추측 금지). */
  rcv : ['받는사람', '받는분', '수취인', '수령인', '수령자명', '고객명', '수취인명', '받는분 성함', '성함'],
  ...
  // ☎ '수령자휴대폰번호' 계열 — 두름 (홍팀장 2026-09-16). '수령자명'과 짝이다.
  tel : ['배송지연락처', '수취인연락처', '받는분 연락처', '받는분연락처', '수취인 연락처', '수취인전화', '수취인 전화', '수취인전화번호', '수취인휴대폰', '수령자휴대폰번호', '수령자휴대폰', '수령자연락처', '수령자 연락처', '연락처', '휴대폰', '전화번호'],
  ...
  /* 🔴 순서가 중요하다 (홍팀장 2026-09-09 여수39): 파일에 「업체명(=자기 사업자명)」과
     「송장업체명(=채널)」이 **둘 다** 오는 곳이 있다. '업체명'을 먼저 집으면 자기 이름이 잡히고,
     buildOut 이 그걸 9칸으로 되돌려 **채널명이 통째로 사라진다**. 송장에 찍힐 이름을 먼저 찾는다. */
  biz : ['송장업체명', '출고지명', '출고지', '업체명'],
...
function headerItems(raw){
  ...
  /* 🏷 값 앞에 말머리가 붙어 오는 업체 — 두름 「상품선택: 반건조 갑 오징어 대 1미」 (홍팀장 2026-09-16).
     우리 상품명은 '주문선택사항' 칸에 있는데 앞에 「상품선택:」이 달려 온다. 이걸 안 떼면
     이름이 통째로 달라져 후보조차 못 찾는다. 규칙 nameStrip 에 적은 말머리만 뗀다(추측 금지 — 적은 것만).
     콜론이 있든 없든, 전각 「：」이어도 뗀다. */
  const NSTRIP = (RULE.nameStrip || []).map(k => S(k)).filter(Boolean)
    .map(k => new RegExp('^\\s*' + k.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '\\s*[:：]?\\s*'));
  const stripLabel = v => { let s = S(v);
    for(const re of NSTRIP){ if(re.test(s)){ s = s.replace(re, '').trim(); break; } }
    return s; };
  ...
  for(let i = hr + 1; i < lines.length; i++){
    ...
    let nm0 = stripLabel(g('name'));
    for(let fi = 0; !nm0 && fi < nameFb.length; fi++) nm0 = stripLabel(dash(c[nameFb[fi]]));   // 비었으면 규칙이 정한 다음 칸
    if(!nm0) continue;
...
function runConvert(append){
  ...
  /* 🏷 대신 발주(마스터)는 **어느 업체 발주인지 먼저** 골라야 읽는다 (홍팀장 2026-09-11 아크미).
     업체마다 파일 읽는 법(업체 규칙 — 아크미는 「옵션(수집)이 상품명」)이 다르고, 그 규칙은 **고른 업체**로 찾는다.
     발주를 넣으면 고른 업체가 비워지는데, 그 상태로 다음 파일을 넣으니 규칙 없이 '상품명' 칸으로 읽혀 조용히 틀렸다.
     머리글만 보고 업체를 추측해 고르지는 않는다 — 사방넷 양식은 여러 업체가 같이 쓴다(엉뚱한 업체로 발주가 간다).
     원문은 칸에 그대로 남으니 업체를 고르고 [변환]만 다시 누르면 된다. */
  if(amMaster() && !(FOR && S(FOR.name))){
    if(logEl) logEl.innerHTML = '<div class="ordwarn">🏷 <b>어느 업체 발주인지 먼저 골라주세요</b> — 업체마다 파일 읽는 법이 달라서, 고른 뒤에 읽어야 상품명이 맞게 들어갑니다.<br>위 업체 칸에서 고른 다음 <b>[변환]</b>을 다시 눌러주세요(파일 내용은 그대로 남아 있습니다).</div>';
    toast('어느 업체 발주인지 먼저 골라주세요');
    return;
  }
...
document.addEventListener('click', e => {
  ...
  if(mk){
    ...
    /* 🪟 창은 **하나만** 연다 (홍팀장 2026-09-03 — "안되는데").
       한 번 눌러 상품 글까지 같이 열려고 window.open 을 두 번 불렀는데, 크롬이 두 번째를 막는다.
