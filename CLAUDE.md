...
| `secretary.html` | **비서** — 업무일지/합포장 기억소/데일리 초안/공유서. 구글시트 서비스계정 연동. 💎고마진 탭의 **[📦 재고 가져오기]** = 마스터 수량시트(월간, 주소는 도구시트 `설정!B1`)의 창고탭 전부에서 `잔여수량`을 읽어 반영 + **수량시트에 없는 상품은 목록에서 뺌**(원본 전체상품원가는 안 건드림). 가져온 값은 도구시트 `재고_캐시` 탭에 저장 → 재접속/다른 PC에서도 마지막 값 표시. ⚠️ **수량시트는 유통사 공용(65명 공유, 소유자 외부)이라 서비스계정에 뷰어 공유가 되어 있어야 동작** — 매월 새 시트로 바뀌면 그 달 시트도 `sheets-writer@baljuseo-sheets.iam.gserviceaccount.com` 에 다시 공유해야 함 |
| `sales.html` | 판매 분석 |
